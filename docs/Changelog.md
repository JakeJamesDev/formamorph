...
- **👤 User-facing**
  ...
  - **Use Summaries in Context** (Settings → Generation, default off; needs Memory Summaries on) — long stories now stay coherent without bloating each request. Recent turns are kept word-for-word, older turns collapse into a compact "story so far" recap built from their summaries, and any past turn your action references is pulled back to full detail. The memory meter breaks out how much of the window the recap and rehydrated turns use. Off by default because it changes what each request sends; with it off, Memory Summaries still only generate and show summaries.
