...
- **👤 User-facing**
  ...
  - **Danbooru tag autocomplete** — the **Image Tags** fields (and the **Generate with AI** prompt) suggest real Danbooru tags as you type, ranked by how commonly they're used, so you complete a tag with a keystroke instead of guessing spellings. Focus an empty field to see the most popular tags; keep typing to narrow. The tag list loads only when you first use a tag field.
