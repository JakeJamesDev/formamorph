...
- **👤 User-facing**
  - **Import tags from an image's metadata** — upload a PNG that embeds its generation prompt (**A1111/Forge**, **InvokeAI**, **NovelAI**, and other JSON-metadata tools) to an entity or location, and Formamorph offers to use that positive prompt as the **Image Tags** — no retyping. Declined images and plain photos are untouched.
