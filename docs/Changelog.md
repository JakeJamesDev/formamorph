...
- **⚙️ Backend / invisible**
  ...
  - **Per-sentence TTS + karaoke plumbing** — narration is now synthesized one audio unit per sentence (an over-long sentence is sub-chunked but keeps a single sentence index), so the playback engine can map the playhead to a sentence and expose `activeSentenceIndex`/`sentenceTexts`. The in-place highlighter paints the active sentence via the browser's CSS Custom Highlight API (no DOM restructuring of the Markdown renderer), with the position→sentence and sentence→DOM-range mapping in pure, unit-tested helpers (`lib/ttsHighlight.ts`, anchor-by-text with an index-segmentation fallback). Settings-only; no world/save-shape change.
...
- **⚙️ Backend / invisible**
  - **The director creates named characters, not props** — when the staged director invents a character it must now give it a concrete, reusable name (so it stays the same character turn to turn instead of drifting "small creature" → "creature" → "forest creature"), and the "no scenery in the cast" rule was reinforced (a forest or a tree is a place, not a cast member). The narration also introduces a not-yet-known character by description and only uses its name once the story has revealed it, so you won't be told a stranger's name before anyone could know it.
