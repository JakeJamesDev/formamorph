...
- **🛠️ Developer tooling**
  ...
  - **Save shape: optional per-playthrough `dictionaries`** — the save envelope (`SaveObject`) gains an **optional** `dictionaries: Dictionary[]` written by the new world-entry dictionary step; older saves lack it and fall back to the world's current books (never clobbered). `saveGame`/`loadGame` carry it across the Gameplay↔GameData boundary via a callback, keeping the contexts decoupled. Pure selection helpers live in `lib/dictionarySelection.ts` (`shouldShowDictionaryStep`, `buildInitialSelection`, `finalizeSelection`). Additive — old saves import unchanged.
