 ...
 * string; move it into `worldOverview.customPlayerVRM` as our `{ data, type }` MediaAsset. Other
 * field defaults are left to `loadWorldData`. Add further `2.0 → 2.x` steps here when the shape
 * changes (a version bump is the user's call — see the export-shape-versioning note).
 */
