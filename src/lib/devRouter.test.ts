...
import { SURFACE_LABELS } from './promptGroups';
...
describe('dev-router coverage guard', () => {
  ...
  it('ledger lists exactly the prompt surfaces the Prompts panel can show', () => {
    expect([...DEV_MODAL_TABS.settingsPromptSurfaces]).toEqual(Object.keys(SURFACE_LABELS));
  });
  ...
  it('registers the modals the router opens', () => {
    ...
    expect(DEV_MODALS).toEqual([
      'settings', 'entity', 'export', 'menu', 'worldEditor', 'intro', 'avatar', 'backup', 'aiSetup', 'entityEditor', 'dictionaryEditor', 'modelDetails', 'community', 'memoryManager', 'profile', 'feedbackHub', 'adminPanel', 'editText', 'location', 'changelog', 'eventAck', 'publish', 'worldPrompts',
    ]);
  });
});
