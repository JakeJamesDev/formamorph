 ...
 * its header the moment narration begins — unless the reader has taken manual control. The body renders full
 * markdown but de-emphasized (muted, smaller, down-scaled headings) so it stays subordinate to the story.
 */
export function ReasoningBlock({ text, ms, active }: { text: string; ms: number; active: boolean }) {
  const [open, setOpen] = useState(active);
  const userToggled = useRef(false);
  // Follow `active` (open while thinking → collapse when narration starts) until the reader clicks the header.
  useEffect(() => {
