...
**Registry-driven enumeration.** The module walks each family's axes from the registry and builds one value per combination. The scoped-token expander stops re-typing content and format ids and reads them from the registry's axes. The Stats block reads its pieces and format from the decoded variant, once, inside the module. The Traits block renders each format from one call. The scene override for the Choices prompt (the patch from 2026-09-23) becomes a call into the module with the in-scene roster substituted, so the override and the base values cannot enumerate different sets.
