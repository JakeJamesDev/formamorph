...
- Any change to the save envelope or the world file shape. The migration rewrites the text of `code` in worlds and in saves; it adds no field. `codeBounds` and `codePins` are as v2 left them.
