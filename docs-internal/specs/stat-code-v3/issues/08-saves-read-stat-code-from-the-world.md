...
- [ ] After a world edit to a stat's code, a loaded save runs the new code on its next turn and on a stats re-roll
- [ ] After a world edit to a stat's name, the sandbox map keys the save's stat by the new name
- [ ] The save's `value`, bounds, `starting`, AI max delta, code bounds, and enabled state are unchanged by the refresh
- [ ] A stat deleted from the world runs its saved code unchanged
- [ ] A world edit that adds a clock variable to a stat's code makes that save's code run every turn from the next turn on
- [ ] The save envelope is byte-for-byte unchanged by a load-and-play cycle, beyond the turn's own state changes
- [ ] Unit tests on the refresh function cover every criterion above; the gameplay-context save round-trip test still passes
