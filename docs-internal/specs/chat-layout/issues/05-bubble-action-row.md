...
- [ ] Latest turn: Re-generate Narration (refresh icon), Re-generate Stats (chart icon), Generate Scene Image (only with no image), Edit, Read Aloud, Copy Text.
- [ ] Past turn: the same without the two re-generate actions, plus Rewind to Here.
- [ ] Write Scene Tags and Re-generate TTS are in the list but do not show as row icons. A More icon on the row opens them. Ticket 06 replaces that popup with the full menu.
