...
20. As a player on a budget-capable target, I want each prompt's Options tab to show the budget slider while the Output row keeps the effort dropdown, so that the exact cap is per prompt and the effort has one visible control.
...
**UI.** The Native Reasoning controls hide behind the existing short note when the record says the model does not reason. The strength dropdown lists the record's accepted levels, or the safe fallback while unknown. The prompt Options control shows the budget slider when the record says the target takes a budget, whichever engine it is, and the effort dropdown otherwise. The Output row always shows the effort dropdown, so the effort a budget-capable target still receives has one visible control. The AI Context viewer's per-request endpoint line shows the effort level and budget tokens the request carried. Request Anatomy is unchanged: it draws prompt text, not request parameters.
